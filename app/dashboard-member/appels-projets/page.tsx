...
export default function AppelsProjets() {
  ...
  const handleAddDocument = () => {
    if (nouveauDocument.nom.trim() && nouveauDocument.fichier) {
      const newDoc = {
        id: Date.now().toString(),
        name: nouveauDocument.nom,
    ...
    <div className="flex h-screen bg-gray-50">
              ...
              <Card>
                <CardContent className="p-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-sm font-medium text-gray-600">
                        {activeTab === "appels" ? "Total appels à projets" : "Total projets"}
                      </p>
                      <p className="text-2xl font-bold text-gray-900">
                        {activeTab === "appels" 
                          ? appelsProjets.length
                      ...
                      <div className="space-y-3">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                          <div>
                            ...
                            <Input
                              placeholder="Nom et prénom du membre permanent"
                              value={nouveauMembrePermanent.nom}
                              onChange={(e) => handleMembrePermanentInputChange("nom", e.target.value)}
                          ...
                          <div>
                            ...
                            <Input
                              ...
                              value={nouveauMembrePermanent.laboratoire}
                              onChange={(e) => handleMembrePermanentInputChange("laboratoire", e.target.value)}
                              className="h-9 text-sm"
                            />
                          </div>
                        </div>
                        <div>
                          ...
                          <Input
                            ...
                            value={nouveauMembrePermanent.etablissement}
                            onChange={(e) => handleMembrePermanentInputChange("etablissement", e.target.value)}
                            className="h-9 text-sm"
                          />
                        </div>
                        <Button
                          type="button"
                          onClick={handleAddMembrePermanent}
                          disabled={!nouveauMembrePermanent.nom.trim() || !nouveauMembrePermanent.laboratoire.trim() || !nouveauMembrePermanent.etablissement.trim()}
                          className="h-9 px-3 bg-uh2c-blue hover:bg-uh2c-blue/90 text-white disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                          <Plus className="h-4 w-4 mr-2" />
                          Ajouter le membre permanent
                        </Button>
                      </div>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="space-y-3">
                    ...
                    <Button
                      ...
                      disabled={!nouveauDocument.nom.trim() || !nouveauDocument.type || !nouveauDocument.fichier}
                      className="h-9 px-3 bg-uh2c-blue hover:bg-uh2c-blue/90 text-white disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      <Plus className="h-4 w-4 mr-2" />
                      Ajouter le document
      ...
      <Dialog open={showNewAppelForm} onOpenChange={setShowNewAppelForm}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="text-xl">Créer un nouvel appel à projets</DialogTitle>
            <DialogDescription>
              Remplissez les informations pour créer un nouvel appel à projets de recherche
            </DialogDescription>
          </DialogHeader>
